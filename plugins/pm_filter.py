...
@Client.on_message(filters.text & filters.pm & filters.incoming & ~filters.command(["verify", "connect", "id"]))
async def search(bot, message):
    f_sub = await force_sub(bot, message)
    if f_sub==False:
       return     
    channels = (await get_pm(message.from_user.id))["channels"]
...
async def recheck(bot, update):
    ...
    channels = (await get_pm(update.message.from_user.id))["channels"]
...
async def request(bot, update):
    ...
    admin = (await get_pm(update.message.from_user.id))["user_id"]
    id    = update.data.split("_")[1]
