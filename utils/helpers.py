...
async def add_pm(user_id, user_name, channels, f_sub, verified):
    data = {
        "_id": user_id,
        "user_name": user_name,
        "channels": channels,
    ...
    try:
        await pm_col.insert_one(data)
    except DuplicateKeyError:
        pass

async def get_pm(id):
    data = {'user_id': id}
    pm = await pm_col.find_one(data)
    return dict(pm)

async def update_pm(id, new_data):
    data = {"_id": id}
    new_value = {"$set": new_data}
    await pm_col.update_one(data, new_value)

async def delete_pm(id):
    data = {"_id": id}
    await pm_col.delete_one(data)
...
async def get_pms():
    count = await pm_col.count_documents({})
    cursor = pm_col.find({})
    result = await cursor.to_list(length=int(count))
    return count, result

async def force_sub(bot, message):
    pm = await get_pm(message.from_user.id)
    f_sub = pm["f_sub"]
    admin = pm["user_id"]
    if f_sub == False:
        return True
    ...
    try:
        ...
        member = await bot.get_chat_member(f_sub, message.from_user.id)
        if member.status == enums.ChatMemberStatus.BANNED:
            await message.reply(f"ꜱᴏʀʀʏ {message.from_user.mention}!\n ʏᴏᴜ ᴀʀᴇ ʙᴀɴɴᴇᴅ ɪɴ ᴏᴜʀ ᴄʜᴀɴɴᴇʟ, ʏᴏᴜ ᴡɪʟʟ ʙᴀɴɴᴇᴅ ꜰʀᴏᴍ ʜᴇʀᴇ ᴡɪᴛʜɪɴ 10 ꜱᴇᴄᴏɴᴅꜱ")
            await asyncio.sleep(10)
            await bot.ban_chat_member(message.chat.id, message.from_user.id)
            return False
    except UserNotParticipant:
